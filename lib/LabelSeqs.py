...
The prgram will stop if the folder labeled already exist.

Supoprt for QIIME label will be added later.
...
def ReLabelFastQ(file_name,label,read_type,input_folder,output_folder='labeled',file_type ='fastq',label_type='both'):
...
def LabelFiles(mapping):
    ...
    for item in mapping:
        count = ReLabelFastQ(item['file'],item['label'],item['read_type'],item['input_folder'],\
                            output_folder=item['output_folder'],file_type=item['file_type'],label_type=item['label_type'])
...
def SplitMapping(mapping_file,input_folder,output_folder='labeled',file_type='fastq',label_type='both',processor=4):
...
#%%++++++++Main Function+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def MainLabelFiles(mapping_file,input_folder,threads=1,output_folder='labeled',file_type='fastq',label_type='both'):
    ...
    if threads == 1:
        ...
        mapping_multithreads = SplitMapping(mapping_file,input_folder,output_folder=output_folder,file_type=file_type,label_type=label_type,processor=threads) 
...
if __name__ == '__main__':
    ...
    start = time.time()
    mapping = 'mapping.txt'
    t = raw_input("Enter thread number: ")
    MainLabelFiles('mapping.txt','scrg',threads=int(t))
    end = time.time()
    print "It used %d seconds to labeled all files."%(end-start)