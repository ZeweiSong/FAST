...
Add Qiime and/or USEARCH labels to the FASTQ/FASTA files. It uses a mapping file generated by generate_mapping.py to find target files in a specified folder.
...
The resulting relabeled files will be stored in a new folder (labeled in default), and can be merged into a single file for quality filter.
...
def main():
    ...
    parser.add_argument("-a", "--fasta", action="store_true", help="Set input file type to FASTA instead of FASTQ")
    ...
    print '%d files has been labeled in %ds.' % (file_num, used_time)
